...
export class HeaderComponent {
  ...
  themeChange(type: string, type1: string, type2: string) {
    ...
    this.elementRef.nativeElement.ownerDocument.documentElement?.setAttribute('data-header-styles', 'color');
    localStorage.setItem('ynex-header-mode', 'color');
    this.elementRef.nativeElement.ownerDocument.documentElement?.setAttribute('data-menu-styles', 'color');
    localStorage.setItem('ynex-menu-mode', 'color');
  ...
  ngOnInit() {
    ...
    this.authService.getUserInfo().subscribe(user => {
      ...
      this.nombre = user.name; // ahora sí, sin error
    });
